...
categories: [Database]
slug: postgresql-shared-buffer-mechanism
---
...
3. **1번 버퍼 검사**: Usage Count가 11로 높습니다. 시계 바늘은 이 카운트를 하나 깎습니다 ($11 -> 10$).
...
1. 버퍼 id: 실제 버퍼가 저장된 버퍼 풀 배열에 해당하는 위치를 저장
2. 상태 플래그: 페이지가 사용되고 있는 상태를 나타내는 플래그
3. 참조 카운트: 메모리에서 페이지가 참조하고 있는 프로세스 갯수, 이 수치로 페이지 교체 대상을 결정

### 상태 플래그

| Name     | Description                                                  |
| :------- | ------------------------------------------------------------ |
| empty    | 버퍼 풀에 올라오지 않은 상태(즉 프로세스 아무도 읽은적이 없는 상태. Usage-count=0, ref-count=0) |
| pinned   | 버퍼 풀 슬롯에 존재하는 상태이고 프로세스가 페이지를  읽고 있는 상태(usage-count >= 1, ref-count >= 1) |
| unpinned | 버퍼 풀 슬롯에 존재하는 상태이지만 프로세스 아무도 읽고 있지 않은 상태(usage-count>=1, ref-count = 0) |

용어 정리를 하면 usage-count는 말그대로 해당 데이터를 읽은 횟수, ref-count는 현재 몇개의 프로세스가 사용중인 알수 있는 수치, 최초 버퍼 풀에 올라오면 두개의 값은 1로 설정된다.
